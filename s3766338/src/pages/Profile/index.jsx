...
const ProfilePage = () => {
  const [profile, setProfile] = useState(
    JSON.parse(localStorage.getItem(typeLocal.ACCOUNT_LOGIN)) || {}
  );
  ...
  const onSubmit = (data) => {
    ...
    axios.put("http://localhost:3001/users/update", data).then((response) => {
      if (response.status === 200) {
        toast.success("You have updated profile successful");
        const accountUpdated = response.data;
        setProfile(accountUpdated);
      } else {
  ...
  const handleDeleteAccount = () => {
    axios
      .delete(
        `http://localhost:3001/users/delete/${profile.id}`
      )
      .then((response) => {
    ...
    <Box>
                  ...
                  <Box sx={{ display: "flex" }}>
                    <Box sx={{ marginRight: 2 }}>
                      <img
                        ...
                        src="http://inspiredluv.com/wp-content/uploads/2016/09/11-beautiful-girl-image.jpg"
                        alt=""
                      />
                    </Box>
      ...
      <Dialog onClose={handleClose} open={open} sx={{ minWidth: "568px" }}>
        ...
        <DialogActions>
          ...
          <Button onClick={handleDeleteAccount} sx={{ color: "#E91E63" }}>
            CONFIRM
          </Button>
        </DialogActions>
      </Dialog>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogContent>
            <Box sx={{ display: "none" }}>
              ...
              />
              <Typography
                sx={{ textAlign: "left", fontSize: "10px", color: "red" }}
              >
          ...
          <DialogActions>
            ...
            <Button type="submit" sx={{ color: "#E91E63" }}>
              CONFIRM
            </Button>
          </DialogActions>
        </form>
